...
extrap_dict = pickle.load(open(os.path.join(pth, 'constant_Emin_50GeV'), 'rb'), encoding='latin1')

def get_weight(E, nu='Nu', current='CC', tgt='p', ext_type='constant', lgE_min_cust=1.68):
    ...
    if current not in ['CC', 'NC']:
        ...
        return 1.
    if tgt not in ['p', 'n']:
        print( "Target nucleon is`t specified correctly! Possible values are: 'p', 'n'.\n Returning weight value = 1.")
        return 1.
...
#     print ( 'lgE_min = ', lgE_min )
    ...
    poly_coef = extrap_dict[nu][current][tgt]['poly_coef']
...
#         w[ext_lgE_reg] = extrap_dict[nu][current]['c']*w[ext_lgE_reg]
        ...
        lin_coef = extrap_dict[nu][current][tgt]['linear']
